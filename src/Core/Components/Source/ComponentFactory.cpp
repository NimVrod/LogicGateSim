...
std::unique_ptr<Component> ComponentFactory::create(
    ...
    int numInputs)
{
    static const std::unordered_map<std::string, std::function<std::unique_ptr<Component>(int, sf::Vector2f, int)>> factoryMap = {
        ...
        {"ClockComponent", [](int id, sf::Vector2f pos, int) { return std::make_unique<ClockComponent>(id, pos); }},
        ...
        {"Input Pin", [](int id, sf::Vector2f pos, int index) { return std::make_unique<InputComponent>(id, pos, index); }},
        {"Output Pin", [](int id, sf::Vector2f pos, int index) { return std::make_unique<OutputComponent>(id, pos, index); }},
        {"InputComponent", [](int id, sf::Vector2f pos, int index) { return std::make_unique<InputComponent>(id, pos, index); }},
        {"OutputComponent", [](int id, sf::Vector2f pos, int index) { return std::make_unique<OutputComponent>(id, pos, index); }}
    };
...
std::unique_ptr<Component> ComponentFactory::createCustom(int id, sf::Vector2f position, const std::string& definitionName) {
